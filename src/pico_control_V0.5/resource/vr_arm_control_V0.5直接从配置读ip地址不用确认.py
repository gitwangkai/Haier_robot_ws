...
def get_vr_server_ip():
    ...
    default_ip = '192.168.1.100'  # 默认IP地址
    ...
    try:
        if os.path.exists(config_file):
            ...
            if saved_ip:
                print(f"从配置文件读取VR服务端IP地址: {saved_ip}")
                return saved_ip
            else:
                print(f"配置文件 {config_file} 为空，使用默认IP地址: {default_ip}")
                return default_ip
        else:
            print(f"配置文件 {config_file} 不存在，使用默认IP地址: {default_ip}")
            return default_ip
    except Exception as e:
        print(f"读取配置文件失败: {e}，使用默认IP地址: {default_ip}")
        return default_ip
