...
#V0.62  加入工作空间检查，末端限定在工作空间内，提升可达性和安全性
import rclpy
...
# 🔧 工作空间边界限制（单位：米）
# 坐标系: X轴=上下(正向上), Y轴=前后(正向后), Z轴=左右(右臂正向右)
LEFT_WORKSPACE_LIMITS = {
    'x': (-0.4, 0.17),      # 上下: 下方0.15m ~ 上方0.43m
    'y': (-0.4, 0.1),       # 前后: 向前0.2m ~ 向后0.3m
    'z': (-0.3, -0.05)      # 左右: 向左最多到0.05 ~ 向右0.15
}
RIGHT_WORKSPACE_LIMITS = {
    'x': (-0.4, 0.17),      # 上下: 下方0.15m ~ 上方0.43m  
    'y': (-0.4, 0.1),       # 前后: 向前0.2m ~ 向后0.3m
    'z': (0.05, 0.3)        # 左右: 向左0.1 ~ 向右0.15
}
...
class VRArmController(Node):
    ...
    def create_arm_models(self):
        # 🔧 修改：使用实际舵机限位转换为理论角度范围
        # 理论角度 = 舵机角度 - offset
        print("🔧 采用实际舵机限位的DH模型，每个关节使用独立的角度范围...")
        
        # 计算每个关节的理论角度限位（弧度）
        left_joint_limits = []
        for i in range(6):
            servo_min, servo_max = self.servo_angle_limits[i]
            offset = self.joint_offsets[i]
            theo_min = servo_min - offset
            theo_max = servo_max - offset
            qlim_rad = (np.radians(theo_min), np.radians(theo_max))
            left_joint_limits.append(qlim_rad)
            print(f"  左臂关节{i+1}: 舵机[{servo_min:.0f}°~{servo_max:.0f}°] → 理论[{theo_min:.0f}°~{theo_max:.0f}°] → [{qlim_rad[0]:.3f}~{qlim_rad[1]:.3f}]rad")
        ...
        for i in range(6):
            ...
            if i in RIGHT_ARM_REVERSE_JOINTS:
                theo_min = offset - servo_max
                theo_max = offset - servo_min
            else:
                theo_min = servo_min - offset
                theo_max = servo_max - offset
            qlim_rad = (np.radians(theo_min), np.radians(theo_max))
            right_joint_limits.append(qlim_rad)
            reverse_mark = " (反转)" if i in RIGHT_ARM_REVERSE_JOINTS else ""
            print(f"  右臂关节{i+1}{reverse_mark}: 舵机[{servo_min:.0f}°~{servo_max:.0f}°] → 理论[{theo_min:.0f}°~{theo_max:.0f}°] → [{qlim_rad[0]:.3f}~{qlim_rad[1]:.3f}]rad")
        
        self.left_arm = DHRobot([
            RevoluteMDH(d=-0.15363, a=0, alpha=0,         qlim=left_joint_limits[0]),
            RevoluteMDH(d=0,        a=0, alpha=np.pi/2,   qlim=left_joint_limits[1]),
            RevoluteMDH(d=-0.26658, a=0, alpha=-np.pi/2,  qlim=left_joint_limits[2]),
            RevoluteMDH(d=0,        a=0, alpha=np.pi/2,   qlim=left_joint_limits[3]),
            RevoluteMDH(d=-0.20024, a=0, alpha=-np.pi/2,  qlim=left_joint_limits[4]),
            RevoluteMDH(d=0,        a=0, alpha=np.pi/2,   qlim=left_joint_limits[5])
        ], name="Left-arm")
        
        self.right_arm = DHRobot([
            RevoluteMDH(d=0.15363,  a=0, alpha=0,         qlim=right_joint_limits[0]),
            RevoluteMDH(d=0,        a=0, alpha=-np.pi/2,  qlim=right_joint_limits[1]),
            RevoluteMDH(d=0.26658,  a=0, alpha=np.pi/2,   qlim=right_joint_limits[2]),
            RevoluteMDH(d=0,        a=0, alpha=-np.pi/2,  qlim=right_joint_limits[3]),
            RevoluteMDH(d=0.20024,  a=0, alpha=np.pi/2,   qlim=right_joint_limits[4]),
            RevoluteMDH(d=0,        a=0, alpha=-np.pi/2,  qlim=right_joint_limits[5])
        ], name="Right-arm")

    def check_workspace_limits(self, target_pose, workspace_limits, arm_name):
        """检查目标位姿是否在工作空间范围内"""
        x, y, z = target_pose
        ...
        violations = []
        if x < x_min:
            violations.append(f"X轴超下限: {x:.3f} < {x_min:.3f} (向下过低)")
        elif x > x_max:
            violations.append(f"X轴超上限: {x:.3f} > {x_max:.3f} (向上过高)")
            
        if y < y_min:
            violations.append(f"Y轴超下限: {y:.3f} < {y_min:.3f} (向前过远)")
        elif y > y_max:
            violations.append(f"Y轴超上限: {y:.3f} > {y_max:.3f} (向后过远)")
            
        if z < z_min:
            violations.append(f"Z轴超下限: {z:.3f} < {z_min:.3f} ({'向左过远' if arm_name == '右臂' else '向右过远'})")
        elif z > z_max:
            violations.append(f"Z轴超上限: {z:.3f} > {z_max:.3f} ({'向右过远' if arm_name == '右臂' else '向左过远'})")
        
        if violations:
            print(f"  ⛔ {arm_name}超出工作空间边界:")
            for v in violations:
                print(f"     {v}")
            return False
        return True
                    ...
                    try:
                        ...
                        if pos_error <= IK_POSITION_TOLERANCE:
                            ...
                            if pos_error > IK_EXECUTION_TOLERANCE:
                                ...
                                # 🔧 新增：检查IK解是否在工作空间内
                                if self.check_workspace_limits(self.left_current_pose, LEFT_WORKSPACE_LIMITS, "左臂"):
                                    new_left_angles[:6] = limited_arm_angles
                                    send_left_cmd = True
                                else:
                                    print(f"  -> ⚠️ IK解超出工作空间，拒绝执行，回滚位姿")
                                    self.left_current_pose = prev_pose
                                    self.left_current_rot = prev_rot
                                    send_left_cmd = False
                    ...
                    try:
                        ...
                        if pos_error <= IK_POSITION_TOLERANCE:
                            ...
                            if pos_error > IK_EXECUTION_TOLERANCE:
                                ...
                                # 🔧 新增：检查IK解是否在工作空间内
                                if self.check_workspace_limits(self.right_current_pose, RIGHT_WORKSPACE_LIMITS, "右臂"):
